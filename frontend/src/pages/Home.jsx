...
export default function Home() {
  ...
  useEffect(()=>{
    const fetchArticles = async()=>{
      return await requestApi("/articles",{
        method:"GET"
      });
    }
    fetchArticles();
  ...
  if(data_?.articles.length === 0) return (
    <div className=' h-screen flex items-center justify-center'>
      <div>
        <h1 className='text-2xl font-bold text-center'>No articles found</h1>
        <span className=' text-gray-500'>You have To subscribe for get articles</span>
      </div>
    </div>
  )
  ...
  return (
    <div className='pt-20 relative'>
        <ArticlesContainer articles={data_?.articles.articles} />
        <button
        onClick={() => {
